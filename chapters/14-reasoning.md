...
RL with Verifiable Rewards is subtly different from learning solely from ground truth answers. In some domains, like mathematics, a single ground truth answer is available to check problems. Originally, RLVR was to be named RL with Ground Truth rewards (RLGT). However, that name didn't generalize well to domains such as code or precise instruction following. In these areas, answers can be verified with a checking function (e.g., a unit test), but there might not be just one 'ground truth' answer.
